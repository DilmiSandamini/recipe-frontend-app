...
const EditRecipe = ({ isOpen, onClose, refreshRecipes, recipe }) => {
    ...
    const handleSubmit = (e) => {
        ...
        data.append('existingImageUrl', recipe.imageUrl || '');
        
        if (imageFile) {
            data.append('image', imageFile);
        }

        api.put(`/catalog/update/${recipe.id}`, data, {
            headers: {
            ...
            .catch(err => {
                ...
                alert("Failed to update recipe. Check backend API.");
            });
    };
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            {showSuccess ? (
                <div className="bg-white p-10 rounded-2xl shadow-2xl flex flex-col items-center transform transition-all duration-300 scale-110">
                    ...
                    <p className="text-gray-600 font-medium text-lg">Recipe Updated Successfully</p>
                </div>
            ) : (
                <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-lg relative max-h-[90vh] overflow-y-auto transition-all duration-300">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="ingredients" required value={formData.ingredients} onChange={handleChange} rows="3"
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"></textarea>
                        </div>
                        <div>
                            <label className="block text-gray-700 font-semibold mb-1">Instructions (One step per line)</label>
                            <textarea name="instructions" required value={formData.instructions} onChange={handleChange} rows="3"
                                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"></textarea>
                        </div>
                        
                        <div>
                            <label className="block text-gray-700 font-semibold mb-1">Change Image (Optional)</label>
                            <div className="flex items-center gap-4">
                                {recipe.imageUrl && !imageFile && (
                                    <img src={recipe.imageUrl} alt="Current" className="w-12 h-12 rounded-lg object-cover border" />
                                )}
                                <input 
                                    type="file" 
                                    accept="image/*" 
